...
class TmClassify:
    def __init__(self):
        ...
        enex = self.encoderextend(en)
        de = self.decoder(enex)
        ...
        self.modelAll.compile(optimizer=op,loss=['binary_crossentropy','mae'], metrics=['accuracy'],loss_weights=[1,0.00001])
        dot_img_file = 'modelATEC.png'
        tf.keras.utils.plot_model(self.modelAll, to_file=dot_img_file, show_shapes=True,expand_nested=False)
        self.pad_param = 5
    ...
    def train(self,start_epoch, max_epoch, batch_size, viz_interval):
        ...
        for epoch in range(start_epoch,max_epoch):
            ...
            for step_index in range(max_step):
                    ...
                    train_loss = self.modelAll.train_on_batch([batch_img1,batch_img2],[batch_target,batch_imgTarget])
                    ...
                    if(step_index%viz_interval==0):
                        ...
                        self.modelAll.layers[4].save_weights('ATencoderEx.Weights.h5')
                        self.modelAll.layers[5].save_weights('ATdiscriminator.Weights.h5')
                        self.modelAll.layers[6].save_weights('ATdecoder.Weights.h5')
                        with train_summary_writer.as_default():
                            images1 = np.reshape(batch_img1, (-1, 224, 224, 3))
                            images2 = np.reshape(batch_img2, (-1, 224, 224, 3))
                            
                            mask1 = self.modelAll.predict([images1,images2])
                            mask2 = self.modelAll.predict([images2,images1])
                            result = mask2[:][0].tolist()
                            print(result)
                            ...
                            tf.summary.image("3 output1", ((mask1[:][1]+1)*127.5).astype('uint8'), max_outputs=20, step=step)
                            tf.summary.image("2 input2", ((images2+1)*127.5).astype('uint8'), max_outputs=20, step=step)
                            tf.summary.image("4 output2", ((mask2[:][1]+1)*127.5).astype('uint8'), max_outputs=20, step=step)
