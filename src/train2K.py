...
class TmClassify:
    ...
    def createModel(self):
        base_model=InceptionV3(input_shape=(224,224,1),weights=None,include_top=False) 
        x=base_model.output
    ...
    def gen_data(self,random_index):
        img_pil = Image.open(os.path.join(r'D:\datasets\LSLOGO\Logo-2K+',self.pathlist[random_index])).convert('L')
    ...
    def train(self,start_epoch, max_epoch, batch_size, viz_interval):
        ...
        for epoch in range(start_epoch,max_epoch):
            ...
            for step_index in range(max_step):
                    batch_img = np.zeros((batch_size,self.input_shape[0],self.input_shape[1]))
                    ...
                    if(step_index%viz_interval==0):
                       self.model.save('2kInceptionLAUG.h5')
