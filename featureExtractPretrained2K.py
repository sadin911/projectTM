...
pkl_filename = "scb2K_no.pkl"
with open(pkl_filename, 'wb') as file:
    pickle.dump(features, file)
    
pkl_filename = "pathlist2K_no.pkl"
with open(pkl_filename, 'wb') as file:
    pickle.dump(pathlist[0:path_num], file)
    
features = pickle.load(open(r'scb2K_no.pkl', 'rb'))
pathlist = pickle.load(open(r'pathlist2K_no.pkl', 'rb'))
...
for n in range(len(features)):
    ...
    Path(f"output2K/{inpath[0]}/{inpath[1]}/{in_name}").mkdir(parents=True, exist_ok=True)
    for i in range(20):
        img = Image.open(os.path.join(r'D:\datasets\LSLOGO\Logo-2K+',pathsTop[i])).convert('RGB')
        filename = os.path.basename(pathsTop[i])
        sc = scoreTop[i]
        img.save(f"output2K/{inpath[0]}/{inpath[1]}/{in_name}/{i}_{sc}.jpg")